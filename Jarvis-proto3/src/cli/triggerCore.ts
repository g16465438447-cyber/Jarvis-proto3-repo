...
export type TriggerMode = "interval" | "watch" | "install-task";
...
export interface TaskInstallSpec {
  executable: "schtasks";
  args: string[];
  taskRunCommand: string;
}

const DEFAULTS: Omit<TriggerCliOptions, "mode"> = {
  request: "",
  execute: false,
  intervalMs: 60_000,
  maxRuns: 0,
...
const SUPPORTED_SCHEDULES: TaskSchedule[] = ["MINUTE", "HOURLY", "DAILY", "WEEKLY", "ONCE", "ONLOGON", "ONSTART"];
const ALLOWED_WEEKDAYS = ["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"];

export function parseTriggerArgs(argv: string[], projectRoot: string): TriggerCliOptions {
  const modeToken = argv[0]?.toLowerCase();
  if (!modeToken || !["interval", "watch", "install-task"].includes(modeToken)) {
    throw new Error(
      "Usage: npm run pilot:trigger -- <interval|watch|install-task> --request \"...\" [--execute] [mode flags]"
    );
  }

  const mode = modeToken as TriggerMode;
  const parsed: TriggerCliOptions = {
    ...DEFAULTS,
    mode
  };
  ...
  for (let i = 1; i < argv.length; i += 1) {
    ...
    switch (token) {
      ...
      case "--max-runs":
        ...
        break;
      case "--path":
        parsed.watchPath = toAbsolutePath(nextValue(), projectRoot);
        break;
      case "--debounce-ms":
...
export function interpolateRequestTemplate(
  template: string,
  event: {
    eventPath: string;
    eventName: string;
...
export function buildTaskInstallSpec(input: {
  taskName: string;
  schedule: TaskSchedule;
  request: string;
  execute: boolean;
  ...
  const args: string[] = ["/Create", "/TN", normalizedTaskName, "/SC", input.schedule, "/TR", taskRunCommand];
...
function buildTaskRunCommand(input: {
  ...
  const nodeExecutable = escapePowerShellSingleQuoted(path.normalize(input.nodeExecutable));
  const pilotEntry = escapePowerShellSingleQuoted(path.normalize(input.pilotEntry));
...
function validateModeOptions(options: TriggerCliOptions): void {
  if (!options.request) {
...
function normalizeSchedule(raw: string): TaskSchedule {
  const normalized = raw.toUpperCase() as TaskSchedule;
